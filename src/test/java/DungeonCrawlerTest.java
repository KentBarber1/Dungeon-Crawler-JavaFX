...
*/
    ...
    */
    ...
    @Test
    //trin's  test that makes sure that monsters killed is
    // counted properly
    public void testMonstersKilled() {
        clickOn("Start");
        clickOn("Name").write("Apollo");
        clickOn("#diff").clickOn("Easy");
        clickOn("#weapon").clickOn("Sword");
        clickOn("Play");
        clickOn("#sword1");
        int k = doGame();
        assertEquals(k, Monster.getKilled());
    }

    @Test
    //trin's test to check that restart works
    public void testRestartAfterEndGame() {
        travelToFB();
